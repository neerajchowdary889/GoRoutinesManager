...
// Service represents a complete service using GoRoutinesManager
type Service struct {
...
func (s *Service) statusHandler(w http.ResponseWriter, r *http.Request) {
	...
	apiCount := s.apiApp.GetLocalManagerCount()
	workerCount := s.workerApp.GetLocalManagerCount()
	totalCount := s.globalMgr.GetGoroutineCount()

	status := map[string]interface{}{
		"service": "GoRoutinesManager Example Service",
		"status":  "running",
...
func main() {
	...
	log.Println("Service is running. Press Ctrl+C to shutdown gracefully.")
